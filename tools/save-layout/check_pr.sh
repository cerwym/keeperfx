...
#
#   check_pr.sh <base commit> <output dir>
#
# Run from the repository root. Writes <output dir>/result.json and
...
python3 "$here/compare.py" "$here/roots.txt" "$out/base.tsv" "$out/head.tsv" \
    "$out/result.json" "$out/summary.md" --flags-tsv "$out/head_flags.tsv" "${lua[@]}"
