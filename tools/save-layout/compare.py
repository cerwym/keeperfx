...
def main():
    ...
    open(a.summary_md, "w", encoding="utf-8").write(render(result))
    print(verdict)
...
def render(r):
    ...
    if v == "identical":
        ...
        L.append("**This PR changes the save format.** Saves, and the other files listed below, "
                 "made by earlier builds will **not load** after it.")
    else:
        L.append("**This PR changes the save format without changing its size.** Files made by earlier "
                 "builds will **load with wrong values, with no error**. Avoid this: keep existing fields "
                 "where they are, or change the size so the loader refuses old files.")
    if r["lua_changed"]:
