# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# PlatformVita.cmake — PlayStation Vita platform setup
...
foreach(_target IN LISTS KFX_TARGETS)
    target_compile_options(${_target} PRIVATE -mcpu=cortex-a9 -mfpu=neon -ffast-math)
endforeach()

# DWARF-4 debug info for crash analysis
if(NOT CMAKE_BUILD_TYPE STREQUAL "Release")
    foreach(_target IN LISTS KFX_TARGETS)
        target_compile_options(${_target} PRIVATE -gdwarf-4)
    endforeach()
endif()
...
foreach(_target IN LISTS KFX_TARGETS)
    target_link_libraries(${_target} PRIVATE vitaGL_imported ${VITASHARK_LIB} ${MATHNEON_LIB})
    add_dependencies(${_target} vitagl_external)
    target_sources(${_target} PRIVATE "${CMAKE_SOURCE_DIR}/src/platform/stack_monitor.c")
endforeach()
...
foreach(_target IN LISTS KFX_TARGETS)
    target_link_libraries(${_target} PRIVATE sceshacccgext_stubs)
    target_link_libraries(${_target} PRIVATE
        SceGxm_stub SceDisplay_stub SceCtrl_stub SceTouch_stub
        ScePower_stub SceAudio_stub SceShaccCg_stub SceKernelDmacMgr_stub)
endforeach()

if(VITA_SCE_DIAG)
    foreach(_target IN LISTS KFX_TARGETS)
        target_link_libraries(${_target} PRIVATE taihen_stub)
    endforeach()
endif()
...
if(VORBISFILE_LIB AND VORBIS_LIB AND OGG_LIB)
    ...
    foreach(_target IN LISTS KFX_TARGETS)
        target_link_libraries(${_target} PRIVATE ${VORBISFILE_LIB} ${VORBIS_LIB} ${OGG_LIB})
    endforeach()
endif()
...
add_library(ffmpeg_vita_stubs STATIC "${CMAKE_SOURCE_DIR}/deps/ffmpeg-vita-stubs/ffmpeg_vita_stubs.c")
foreach(_target IN LISTS KFX_TARGETS)
    target_link_libraries(${_target} PRIVATE
        -Wl,--start-group
            ${AVFORMAT_LIB} ${AVCODEC_LIB} ${AVUTIL_LIB} ${SWRESAMPLE_LIB} ${MP3LAME_LIB}
            pthread SceAudiodec_stub ffmpeg_vita_stubs
        -Wl,--end-group)
    target_link_libraries(${_target} PRIVATE centijson_static centitoml)
endforeach()
...
foreach(_target IN LISTS KFX_TARGETS)
    target_include_directories(${_target} PRIVATE
        ...
        ${zlib_src_SOURCE_DIR}/contrib)
    target_link_libraries(${_target} PRIVATE vita_spng vita_minizip vita_zlib)
endforeach()
...
if(LUAJIT_LIB AND LUAJIT_INCLUDE_DIR)
    foreach(_target IN LISTS KFX_TARGETS)
        target_include_directories(${_target} PRIVATE ${LUAJIT_INCLUDE_DIR})
        ...
        target_link_libraries(${_target} PRIVATE ${LUAJIT_LIB} dl taihen_stub SceSblSsMgr_stub)
    endforeach()
...
if(VITA_UVDB)
    ...
    # vita-uvdb only needed on the main (non-hvlog) binary
