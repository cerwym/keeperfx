/*
 * vita-uvdb stdio redirection
 * Original: https://github.com/sleirsgoevy/vita-uvdb (sleirsgoevy)
 *
 * Redirects stdout/stderr through GDB remote syscalls so printf()
 * output appears in the GDB console.
 */
...
// unfortunately, newlib does not export dup2. this is a hack, but
// probably relatively safe one
// would be nice if newlib supported "custom" fds with user-provided
// vtables, this way we could avoid threading at all
...
{
    int source_pipe = (int)(uintptr_t)arg;
    char buf[1024];
    for(;;)
    {
        ...
        {
            size_t pos = 0;
            while(pos < (size_t)chk)
            {
                ssize_t chk2 = uvdb_remote_syscall("write", 3, 1, buf + pos, chk - pos);
                if(chk2 <= 0)
                    break;
                pos += chk2;
            }
...
int uvdb_redirect_stdio(void)
{
    ...
    if(socketpair(AF_INET, SOCK_STREAM, 0, pp) && pipe(pp))
        return -1;
    pthread_t pth;
    if(pthread_create(&pth, 0, redir_thread, (void*)(uintptr_t)pp[1]))
        return -1;
