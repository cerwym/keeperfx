...
TbBool save_game_chunks(TbFileHandle fhandle, struct CatalogueEntry *centry)
{
    ...
    // Adding Achievement data chunk
    {
        struct AchievementTracker* tracker = &achievement_tracker;
        hdr.id = SGC_AchievementData;
        hdr.ver = 0;
        hdr.len = sizeof(struct AchievementTracker);
        if (LbFileWrite(fhandle, &hdr, sizeof(struct FileChunkHeader)) == sizeof(struct FileChunkHeader))
        if (LbFileWrite(fhandle, tracker, sizeof(struct AchievementTracker)) == sizeof(struct AchievementTracker))
...
int load_game_chunks(TbFileHandle fhandle, struct CatalogueEntry *centry)
{
    ...
    while (!LbFileEof(fhandle))
    {
        ...
        case SGC_LuaData:
            ...
            case SGC_AchievementData:
            {
                ...
                if (LbFileRead(fhandle, tracker, sizeof(struct AchievementTracker)) == sizeof(struct AchievementTracker)) {
                    chunks_done |= SGF_AchievementData;
    ...
    // Achievement data is optional for backward compatibility, not precious, may remove later
