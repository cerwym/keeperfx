...
void draw_battle_head(struct Thing *thing, long scr_x, long scr_y, int units_per_px)
{
    ...
    if ((thing->creature.health_bar_turns) && ((get_gameturn() % (2 * gui_blink_rate)) >= gui_blink_rate)) {
        UIRenderer_SubmitPanelSpriteRawColored(curscr_x, curscr_y, ps_units_per_px, spr, player_flash_colours[get_player_color_idx(thing->owner)], 0);
    } else {
        UIRenderer_SubmitPanelSpriteRaw(curscr_x, curscr_y, ps_units_per_px, spr, 0);
    }
