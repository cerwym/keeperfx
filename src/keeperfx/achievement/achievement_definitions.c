...
int load_campaign_achievements(struct GameCampaign* camp)
{
    ...
    JUSTLOG("No achievements.cfg found for campaign '%s'", camp->name);
    return 0;
}
...
int load_achievements_config(const char* fname, struct GameCampaign* camp)
{
    ...
    while (*pos != '\0')
    {
        ...
        JUSTLOG("Parser: Read command '%s' (len=%d, cmd_num=%d)", cmd, (int)strlen(cmd), cmd_num);
        if (cmd_num == 0 && strlen(cmd) > 0)
        {
            JUSTLOG("Parser: Unmatched command bytes: ");
            for (int j = 0; cmd[j] != '\0'; j++)
            {
                JUSTLOG("  [%d] = 0x%02X ('%c')", j, (unsigned char)cmd[j], cmd[j] >= 32 ? cmd[j] : '?');
            }
        }
        ...
        {
            JUSTLOG("Parser: Found ACHIEVEMENT command, in_achievement=%d", in_achievement);
            if (in_achievement)
            {
            ...
            ach_def.id[i] = '\0';
            JUSTLOG("Parser: Starting achievement '%s'", ach_def.id);
        }
        else if (cmd_num == 2) ///< END_ACHIEVEMENT
        {
            JUSTLOG("Parser: Found END_ACHIEVEMENT, in_achievement=%d, ID=%s", in_achievement, in_achievement ? ach_def.id : "NONE");
            if (!in_achievement)
            {
            ...
            {
                JUSTLOG("Parser: Successfully registered achievement '%s'", ach_def.id);
                loaded_count++;
            }
            else
            {
                WARNLOG("Parser: Failed to register achievement '%s'", ach_def.id);
            }
