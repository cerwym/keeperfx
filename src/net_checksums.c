...
static TbBigChecksum compute_player_checksum(struct PlayerInfo *player) {
    if ((player->allocflags & PlaF_CompCtrl) != 0 || !camera_is_active(player->id_number)) {
        return 0;
    }
    struct Coord3d* mappos = &(camera_get_active(player->id_number)->mappos);
    TbBigChecksum checksum = 0;
...
void update_turn_checksums(void) {
    ...
    if ((game.system_flags & GSF_NetworkActive) != 0) {
        ...
        for (int i = 0; i < PLAYERS_COUNT; i++) {
            struct PlayerInfo* player = get_player(i);
            if (!player_exists(player) || ((player->allocflags & PlaF_CompCtrl) != 0) || !camera_is_active(player->id_number)) {
                continue;
            }
            ...
            player_snapshot->mappos = camera_get_active(player->id_number)->mappos;
