...
void gui_area_new_normal_button(struct GuiButton *gbtn)
{
    ...
    if ((gbtn->flags & LbBtnF_Enabled) != 0)
    {
        ...
        if ((!gbtn->button_state_left_pressed) && (!gbtn->button_state_right_pressed))
            i = 1;
        draw_gui_panel_sprite_left(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, gbtn->sprite_idx+i, 0);
    } else
    {
        draw_gui_panel_sprite_rmleft(gbtn->scr_pos_x, gbtn->scr_pos_y, ps_units_per_px, gbtn->sprite_idx+1, 12, 0);
    }
    SYNCDBG(12,"Finished");
}
...
void frontend_draw_scroll_box_tab(struct GuiButton *gbtn)
{
    ...
    UIRenderer_SubmitPanelSpriteRaw(pos_x, pos_y, fs_units_per_px, spr, 0);
