...
void go_on_then_activate_the_event_box(PlayerNumber plyr_idx, EventIndex evidx)
{
    ...
    if (is_my_player_number(plyr_idx))
    {
        ...
        case EvKind_AchievementUnlocked:
        {
            ...
            extern struct Achievement achievements[];
            ...
            if (ach_idx >= 0 && ach_idx < achievements_count)
            {
                ...
                /// Resolve strings at display time
                ...
                // Tooltip shows "Achievement Unlocked: <name>"
                const char* achievement_unlocked_str = get_string(GUIStr_EventAchievementUnlockedDesc);
                snprintf(game.evntbox_text_buffer, sizeof(game.evntbox_text_buffer), "%s: %s", achievement_unlocked_str, ach_name);
                // Full event message shows "<name>: <description>"
                snprintf(game.evntbox_scroll_window.text, sizeof(game.evntbox_scroll_window.text), "%s: %s", ach_name, ach_desc);
                JUSTLOG("Achievement unlocked event displayed: %s", ach_name);
            }
            else
            {
