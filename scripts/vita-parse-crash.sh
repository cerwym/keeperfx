...
#
# Usage:
#   ./scripts/vita-parse-crash.sh                          # auto-latest from Vita
#   ./scripts/vita-parse-crash.sh --interactive             # pick a dump
#   ./scripts/vita-parse-crash.sh --dump out/vita-dumps/crash.psp2dmp
#   ./scripts/vita-parse-crash.sh --vita-ip 192.168.0.100
set -euo pipefail
...
docker run --rm \
    ...
    -w /src \
    "$VITASDK_IMAGE" \
    bash /src/scripts/vita_crash/docker-entry.sh "${TOOL_ARGS[@]}"
