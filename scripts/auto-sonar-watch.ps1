...
function Test-CompileCommandsReady {
    ...
    $cmd = "docker compose -f '$WorkspaceFolder/build/docker/compose.yml' run --rm linux bash -c 'cmake --preset linux-x64-release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON'"
...
function Update-CppcheckReport {
    ...
    $cmd = "docker compose -f '$WorkspaceFolder/build/docker/compose.yml' run --rm linux bash -c 'cmake --preset linux-x64-release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON && mkdir -p out && cppcheck --project=out/build/linux-x64-release/compile_commands.json --xml --xml-version=2 --enable=warning,style,performance,portability,information --inconclusive --suppress=missingIncludeSystem -j 4 2>out/cppcheck.xml || true'"
    Invoke-Expression $cmd | Out-Null
}

function Invoke-SonarScan {
    Write-Host "[auto-sonar] running sonar scan..." -ForegroundColor Cyan
    $cmd = "docker compose -f '$WorkspaceFolder/build/docker/compose.yml' run --rm -e SONAR_HOST_URL='$SonarHostUrl' sonarscanner sh -c 'sonar-scanner -Dproject.settings=sonar-project.properties -Dsonar.projectKey=$SonarProjectKey -Dsonar.token=$SonarToken'"
    Invoke-Expression $cmd | Out-Null
