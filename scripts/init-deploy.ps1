...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts/init-deploy.ps1 -DungeonKeeperPath "C:\Games\Dungeon Keeper"
...
#   powershell -ExecutionPolicy Bypass -File scripts/init-deploy.ps1 -KeeperFxReleasePath "C:\Downloads\kfx"
#
# Parameters:
...
$composeFile = Join-Path $ws "build\docker\compose.yml"
$deployDir = Join-Path $ws ".deploy"
$requiredDkList = Join-Path $ws "docs\files_required_from_original_dk.txt"
$dkDockerfile = Join-Path $ws "build\docker\dk-originals\Dockerfile"
$runtimeDockerfile = Join-Path $ws "build\docker\kfx-runtime-assets\Dockerfile"
...
function Build-RuntimeAssetsInDocker {
    ...
    Write-Host "Generating pkg runtime assets in build/docker/linux ..." -ForegroundColor Cyan
